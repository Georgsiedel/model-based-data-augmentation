    ...
    def get_value(self, corruption_name, severity):
        ...
        try:
            ...
            if corruption_name in self.df["corruption_name"].values:
                ...
                value = self.df.loc[
                    self.df["corruption_name"] == corruption_name, severity
                ].values[0]
                return float(value)
            else:
                return None
...
class Checkpoint:
    ...
    def __init__(
        ...
        self.final_model_path = os.path.abspath(
    ...
    def load_model(
        ...
        if type == "standard":
            filtered_state_dict = {
                ...
                if "deepaugment_instance" not in k
            }
        ...
        if swa_model != None:
            swa_filtered_state_dict = {
                k: v
                for k, v in checkpoint["swa_model_state_dict"].items()
                if "deepaugment_instance" not in k
            }
...
class TrainTracking:
    def __init__(
        ...
        self.learningcurve_path = os.path.abspath(
            f"results/{self.dataset}/{self.modeltype}/config{self.experiment}_"
            f"learning_curve_run_{self.run}.svg"
        )
        self.config_src_path = os.path.abspath(
            f"./experiments/configs/config{self.experiment}.py"
        )
        self.config_dst_path = os.path.abspath(
    ...
    def print_results(self):
        if (
            not self.elapsed_time
        ):  # if we train for 0 epochs (just loading pretrained model)
            return
        print(
            "Total training time: ",
...
class TestTracking:
    ...
    def create_report(self):
        ...
        if self.eval_count > 1:
            column_string = np.append(
                column_string, [f"config_{self.experiment}_std"], axis=0
            )
            for idr in range(self.runs):
                column_string = np.append(
                    column_string, [f"config_{self.experiment}_run_{idr}"], axis=0
                )
