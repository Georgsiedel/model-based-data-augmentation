...
def str2bool(v):
    ...
    if v.lower() in ("yes", "true", "t", "y", "1"):
        ...
        raise argparse.ArgumentTypeError(
            f"Error: Boolean value expected for argument {v}."
        )
...
class str2dictAction(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        try:
            ...
            raise argparse.ArgumentTypeError(
                f"Invalid dictionary format: {values}"
            ) from e
...
def plot_images(
    number, mean, std, images, corrupted_images=None, second_corrupted_images=None
):
    ...
    fig, axs = plt.subplots(
        number, columns, figsize=(2 * col_width, number * row_height), squeeze=False
    )

    images = images.cpu()
    corrupted_images = (
        corrupted_images.cpu() if corrupted_images is not None else corrupted_images
    )
    second_corrupted_images = (
        second_corrupted_images.cpu()
        if second_corrupted_images is not None
        else second_corrupted_images
    )
...
def calculate_steps(
    ...
    if swa == True:
        total_validsteps = (
            validsteps_per_epoch * int((2 - swa_start_factor) * epochs) + warmupepochs
        )
    else:
    ...
    if swa == True:
        started_swa_epochs = (
            start_epoch - warmupepochs - int(swa_start_factor * epochs)
            if start_epoch - warmupepochs - int(swa_start_factor * epochs) > 0
            else 0
        )
...
class Checkpoint:
    ...
    def load_model(
        self, model, swa_model, optimizer, scheduler, swa_scheduler, type="standard"
    ):
    ...
    def save_checkpoint(
        self, model, swa_model, optimizer, scheduler, swa_scheduler, epoch
    ):
...
class TrainTracking:
    def __init__(
        self, dataset, modeltype, lrschedule, experiment, run, validonc, validonadv, swa
    ):
    ...
    def load_learning_curves(self):
        learning_curve_frame = pd.read_csv(
            os.path.abspath(
        ...
        if self.swa["apply"] == True:
            ...
            if self.validonc == True:
                valid_accs_robust_swa = learning_curve_frame.iloc[
                    :, columns + 1
                ].values.tolist()
                columns = columns + 1
            if self.validonadv == True:
                valid_accs_adv_swa = learning_curve_frame.iloc[
                    :, columns + 1
                ].values.tolist()
    ...
    def save_learning_curves(self):
        ...
        columns = 4
        if self.validonc == True:
            learning_curve_frame.insert(
                columns, "valid_accuracy_robust", self.valid_accs_robust
            )
            columns = columns + 1
        if self.validonadv == True:
            learning_curve_frame.insert(
                columns, "valid_accuracy_adversarial", self.valid_accs_adv
            )
            columns = columns + 1
        if self.swa["apply"] == True:
            learning_curve_frame.insert(
                columns, "valid_accuracy_swa", self.valid_accs_swa
            )
            if self.validonc == True:
                learning_curve_frame.insert(
                    columns + 1, "valid_accuracy_robust_swa", self.valid_accs_robust_swa
                )
                columns = columns + 1
            if self.validonadv == True:
                learning_curve_frame.insert(
                    columns + 1,
                    "valid_accuracy_adversarial_swa",
                    self.valid_accs_adv_swa,
                )
            result_dir = f"./results/{self.dataset}/{self.modeltype}"
            if not os.path.exists(result_dir):
                os.makedirs(result_dir)
        learning_curve_frame.to_csv(
            os.path.abspath(
                f"./results/{self.dataset}/{self.modeltype}/config{self.experiment}_"
                f"learning_curve_run_{self.run}.csv"
            ),
        ...
        if self.swa["apply"] == True:
            swa_diff = [
                self.valid_accs_swa[i]
                if self.valid_accs[i] != self.valid_accs_swa[i]
                else None
                for i in range(len(self.valid_accs))
            ]
            plt.plot(x, swa_diff, label="SWA Validation Accuracy")
            if self.validonc == True:
                swa_robust_diff = [
                    self.valid_accs_robust_swa[i]
                    if self.valid_accs_robust[i] != self.valid_accs_robust_swa[i]
                    else None
                    for i in range(len(self.valid_accs_robust))
                ]
                plt.plot(x, swa_robust_diff, label="SWA Robust Validation Accuracy")
            if self.validonadv == True:
                swa_adv_diff = [
                    self.valid_accs_adv_swa[i]
                    if self.valid_accs_adv[i] != self.valid_accs_adv_swa[i]
                    else None
                    for i in range(len(self.valid_accs_adv))
                ]
        ...
        plt.savefig(
            os.path.abspath(
                f"results/{self.dataset}/{self.modeltype}/config{self.experiment}_learning_curve_run_{self.run}.svg"
            )
        )
        plt.close()
    ...
    def save_config(self):
        shutil.copyfile(
            os.path.abspath(f"./experiments/configs/config{self.experiment}.py"),
            os.path.abspath(
    ...
    def print_results(self):
        print(
            "Maximum (non-SWA) validation accuracy of",
            max(self.valid_accs),
            "achieved after",
        ...
        if self.validonc:
            print(
                "Maximum (non-SWA) robust validation accuracy of",
        ...
        if self.validonadv:
            print(
                "Maximum (non-SWA) adversarial validation accuracy of",
...
class TestTracking:
    def __init__(
        ...
        test_on_c,
        calculate_adv_distance,
        calculate_autoattack_robustness,
        train_corruptions,
        test_corruptions,
        adv_distance_params,
    ):
        ...
        self.results_folder = (
            f"./results/{self.dataset}/{self.modeltype}/config{self.experiment}"
        )
        ...
        if calculate_adv_distance:
            self.adv_count = (
                len(self.adv_distance_params["norm"])
                * (2 + len(self.adv_distance_params["clever_samples"]))
                + 1
            )
    ...
    def create_report(self):
        ...
        for idm in range(self.model_count):
            for ide in range(self.test_count):
                self.avg_test_metrics[ide, idm] = self.all_test_metrics[
                    ide, idm, :
                ].mean()
                self.std_test_metrics[ide, idm] = self.all_test_metrics[
                    ide, idm, :
                ].std()
                self.max_test_metrics[ide, idm] = self.all_test_metrics[
                    ide, idm, :
                ].max()
        ...
        if self.combine_train_corruptions == True:
            ...
            train_corruptions_string = np.array(
                [",".join(map(str, row.values())) for row in self.train_corruptions]
            )
        ...
        if self.test_on_c == True:
            ...
            test_corruptions_string = np.append(
                test_corruptions_string, test_corruptions_bar_label, axis=0
            )
            test_corruptions_string = np.append(
                test_corruptions_string,
                [
                    "Acc_C-all-19",
                    "Acc_C-original-15",
                    "Acc_C-bar-10",
                    "Acc_all-ex-pixelwise-noise-24",
                    "RMSCE_C",
                ],
                axis=0,
            )
        ...
        if self.calculate_adv_distance == True:
            test_corruptions_string = np.append(
                test_corruptions_string, ["Acc_from_adv_dist_calculation"]
            )
            for _, n in enumerate(self.adv_distance_params["norm"]):
                test_corruptions_string = np.append(
                    ...
                    [
                        ...
                        f"{n}-norm-Mean_adv_dist_without_misclassifications",
                    ],
                    axis=0,
                )
        ...
        if self.combine_test_corruptions == True:
            test_corruptions_string = np.append(
                test_corruptions_string, ["Combined_Noise"]
            )
        else:
            ...
            test_corruptions_string = np.append(
                test_corruptions_string, test_corruptions_labels
            )

        avg_report_frame = pd.DataFrame(
            self.avg_test_metrics,
            index=test_corruptions_string,
            columns=train_corruptions_string,
        )
        avg_report_frame.to_csv(
            os.path.abspath(f"{self.results_folder}_metrics_test_avg.csv"),
            index=True,
            header=True,
            sep=";",
            float_format="%1.4f",
        ...
        if self.runs >= 2:
            max_report_frame = pd.DataFrame(
                self.max_test_metrics,
                index=test_corruptions_string,
                columns=train_corruptions_string,
            )
            std_report_frame = pd.DataFrame(
                self.std_test_metrics,
                index=test_corruptions_string,
                columns=train_corruptions_string,
            )
            max_report_frame.to_csv(
                os.path.abspath(f"{self.results_folder}_metrics_test_max.csv"),
                index=True,
                header=True,
                sep=";",
                float_format="%1.4f",
            ...
            std_report_frame.to_csv(
                os.path.abspath(f"{self.results_folder}_metrics_test_std.csv"),
                index=True,
                header=True,
                sep=";",
                float_format="%1.4f",
    ...
    def initialize(self, run, model):
        ...
        if self.combine_train_corruptions:
            ...
            print(
                f"Run {run}, evaluating model trained on noise of type:",
                train_corruption,
            )
            self.fileaddition = (
    ...
    def track_results(self, new_results):
        ...
        self.all_test_metrics[: len(self.accs), self.model, self.run] = np.array(
            self.accs
        )
    ...
    def save_adv_distance(self, dist_sorted, adv_distance_params):
        ...
        for id, n in enumerate(adv_distance_params["norm"]):
            ...
            for j, (batches, samples) in enumerate(
                ...
                adv_fig.savefig(
                    os.path.abspath(
                        f"results/{self.dataset}/{self.modeltype}/config{self.experiment}{self.fileaddition}run"
        ...
        adv_distance_frame.to_csv(
            os.path.abspath(
                f"./results/{self.dataset}/{self.modeltype}/config{self.experiment}{self.fileaddition}"
