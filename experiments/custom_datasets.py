...
def custom_collate_fn(
    batch,
    batch_transform_orig,
    batch_transform_gen,
    image_transform_orig,
    image_transform_gen,
    generated_ratio,
    batchsize,
):
    ...
    batch_inputs[: -int(generated_ratio * batchsize)] = batch_transform_orig(
        batch_inputs[: -int(generated_ratio * batchsize)]
    )
    batch_inputs[-int(generated_ratio * batchsize) :] = batch_transform_gen(
        batch_inputs[-int(generated_ratio * batchsize) :]
    )

    for i in range(len(batch_inputs)):
        batch_inputs[i] = (
            image_transform_orig(batch_inputs[i])
            if i < (len(batch_inputs) - int(generated_ratio * batchsize))
            else image_transform_gen(batch_inputs[i])
        )
...
class StylizedTensorDataset(Dataset):
    def __init__(self, dataset, stylized_images, stylized_indices):
        ...
        self.index_map = {
            orig_idx.item(): i for i, orig_idx in enumerate(stylized_indices)
        }
...
class CustomDataset(Dataset):
    def __init__(self, np_images, testset, resize, preprocessing):
        # Load images
        self.np_images = (
            np.memmap(np_images, dtype=np.float32, mode="r")
            if isinstance(np_images, str)
            else np_images
        )
        self.resize = resize
...
class ReproducibleBalancedRatioSampler(Sampler):
    ...
    def __iter__(self):
        ...
        original_perm, generated_perm = self.generate_indices_order(
            self.num_original, self.num_generated, self.current_epoch
        )
        self.current_epoch += 1

        batch_starts = range(
            0, self.size, self.batch_size
        )  # Start points for each batch
        for i, start in enumerate(batch_starts):
            # Slicing the permutation to get batch indices, avoiding going out of bound
            original_indices = original_perm[
                min(i * self.num_original_batch, self.num_original) : min(
                    (i + 1) * self.num_original_batch, self.num_original
                )
            ]
            generated_indices = generated_perm[
                min(i * self.num_generated_batch, self.num_generated) : min(
                    (i + 1) * self.num_generated_batch, self.num_generated
                )
            ]
...
class GroupedAugmentedDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        generated_dataset,
        transforms_basic,
        transforms_batch_gen,
        transforms_batch_orig,
        transforms_iter_orig_after_style,
        transforms_iter_gen_after_style,
        transforms_iter_orig_after_nostyle,
        transforms_iter_gen_after_nostyle,
        robust_samples=0,
        epoch=0,
    ):
        ...
        if transforms_batch_gen:
            self.cache_size_gen = int(
                transforms_batch_gen.batch_size / transforms_batch_gen.stylized_ratio
            )
        else:
            self.cache_size_gen = 1
        if transforms_batch_orig:
            self.cache_size_orig = int(
                transforms_batch_orig.batch_size / transforms_batch_orig.stylized_ratio
            )
        else:
    ...
    def set_epoch(self, epoch):
        ...
        """
        self.original_perm, self.generated_perm = self.generate_indices_order(
            self.num_original, self.num_generated, epoch
        )
    ...
    def __getitem__(self, idx):
        ...
        if idx < self.num_original:
            ...
            dataset_specific_index = (
                idx - self.num_original
            )  # for generated images, adjust index
        ...
        transform_iter = (
            transforms_iter_after_style if style_flag else transforms_iter_after_nostyle
        )
...
class BalancedRatioSampler(Sampler):
    ...
    def __iter__(self):
        ...
        # generated permutation requires generated images appended to the back of the dataset!
        print(
            f"num_original: {self.num_original}, num_generated: {self.num_generated}, batch_size: {self.batch_size}, num_generated_batch: {self.num_generated_batch}, num_original_batch: {self.num_original_batch}, generated_ratio: {self.generated_ratio}"
        )
...
class AugmentedDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self,
        stylized_original_dataset,
        stylized_generated_dataset,
        style_mask,
        transforms_basic,
        transforms_orig_after_style,
        transforms_gen_after_style,
        transforms_orig_after_nostyle,
        transforms_gen_after_nostyle,
        robust_samples=0,
    ):
        ...
        self.num_original = (
            len(stylized_original_dataset) if stylized_original_dataset else 0
        )
        self.num_generated = (
            len(stylized_generated_dataset) if stylized_generated_dataset else 0
        )
        self.total_size = self.num_original + self.num_generated
    ...
    def __getitem__(self, idx):
        ...
        if idx < self.num_original:
            ...
            aug = (
                ...
                if is_stylized
                else self.transforms_orig_after_nostyle
            )
        else:
            ...
            aug = (
                ...
                if is_stylized
                else self.transforms_gen_after_nostyle
            )
