...
parser.add_argument(
    ...
    metavar="KEY=VALUE",
    help="dictionary for type of noise, epsilon value, "
...
parser.add_argument(
    ...
    default="MultiStepLR",
    type=str,
    help="Learning rate scheduler from pytorch.",
)
parser.add_argument(
    "--lrparams",
    default={"milestones": [85, 95], "gamma": 0.2},
    type=str,
    action=utils.str2dictAction,
    metavar="KEY=VALUE",
    help="parameters for the learning rate scheduler",
)
parser.add_argument(
    "--earlystop",
    type=utils.str2bool,
    ...
    const=False,
    default=False,
    help="Use earlystopping after some epochs (patience) of no increase in performance",
)
parser.add_argument(
    "--earlystopPatience",
    default=15,
...
parser.add_argument(
    "--optimizer", default="SGD", type=str, help="Optimizer from torch.optim"
)
parser.add_argument(
    "--optimizerparams",
    default={"momentum": 0.9, "weight_decay": 5e-4},
    type=str,
    action=utils.str2dictAction,
    metavar="KEY=VALUE",
    help="parameters for the optimizer",
)
parser.add_argument(
    "--modeltype",
    default="wideresnet",
...
parser.add_argument(
    ...
    default="TrivialAugmentWide",
    type=str,
    help="augmentation scheme",
)
parser.add_argument(
    "--train_aug_strat_gen",
    default="TrivialAugmentWide",
...
parser.add_argument(
    ...
    type=str,
    help="loss function to use, chosen from torch.nn loss functions",
)
parser.add_argument(
    "--lossparams",
    default={},
...
parser.add_argument(
    ...
    action=utils.str2dictAction,
    metavar="KEY=VALUE",
    help="parameters for the trades loss function",
)
parser.add_argument(
    "--robust_loss",
    type=utils.str2bool,
...
parser.add_argument(
    "--robust_lossparams",
    default={"num_splits": 3, "alpha": 12},
    type=str,
    action=utils.str2dictAction,
    metavar="KEY=VALUE",
    help="parameters for the robust loss function. If 3, JSD will be used.",
)
parser.add_argument(
    "--mixup",
    default={"alpha": 0.2, "p": 0.0},
    type=str,
    action=utils.str2dictAction,
    metavar="KEY=VALUE",
...
parser.add_argument(
    "--manifold",
    default={"apply": False, "noise_factor": 4},
    type=str,
    action=utils.str2dictAction,
    metavar="KEY=VALUE",
...
parser.add_argument(
    ...
    default=1,
    type=int,
    help="How many of the training noise values should "
...
parser.add_argument(
    ...
    default=1,
    type=int,
    help="default is 1 for 32px (CIFAR10), "
...
parser.add_argument(
    ...
    default=8,
    type=int,
    help="batchsize, for which a new corruption type is sampled. "
...
parser.add_argument(
    "--swa",
    default={"apply": True, "start_factor": 0.85, "lr_factor": 0.2},
    type=str,
    action=utils.str2dictAction,
    metavar="KEY=VALUE",
    help="start_factor defines when to start weight "
...
parser.add_argument(
    "--noise_patch_scale",
    default={"lower": 0.3, "upper": 1.0},
    type=str,
    action=utils.str2dictAction,
    metavar="KEY=VALUE",
    help="bounds of the scale to choose the area ratio of the image from, which "
...
parser.add_argument(
    ...
    default=0.0,
    type=float,
    help="ratio of synthetically generated images mixed into every training batch",
)
parser.add_argument(
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        with torch.amp.autocast(device_type=device):
            outputs, mixed_targets = model(
                inputs,
                targets,
                criterion.robust_samples,
                train_corruptions,
                args.mixup["alpha"],
                args.mixup["p"],
                args.manifold["apply"],
                args.manifold["noise_factor"],
                args.cutmix["alpha"],
                args.cutmix["p"],
                args.minibatchsize,
                args.concurrent_combinations,
                args.noise_sparsity,
        ...
        if criterion.robust_samples >= 1:
            mixed_targets = torch.cat(
                [mixed_targets] * (criterion.robust_samples + 1), 0
            )
        ...
        pbar.set_description(
            "[Train] Loss: {:.3f} | Acc: {:.3f} ({}/{})".format(
                avg_train_loss, 100.0 * correct / total, correct, total
            )
        )
        pbar.update(1)
...
def valid_epoch(pbar, net):
    net.eval()
    with torch.no_grad():
        test_loss, correct, total, avg_test_loss, adv_acc, acc_c, adv_correct = (
            0,
            0,
            0,
            0,
        ...
        for batch_idx, (inputs, targets) in enumerate(validationloader):
            ...
            with torch.amp.autocast(device_type=device):
                if args.validonadv == True:
                    adv_inputs, outputs = fast_gradient_validation(
                        model_fn=net,
                        eps=8 / 255,
                        x=inputs,
                        y=None,
            ...
            pbar.set_description(
                "[Valid] Loss: {:.3f} | Acc: {:.3f} ({}/{}) | Adversarial Acc: {:.3f}".format(
                    avg_test_loss,
                    100.0 * correct / total,
                    correct,
                    total,
        ...
        if args.validonc == True:
            pbar.set_description(
                "[Valid] Robust Accuracy Calculation. Last Robust Accuracy: {:.3f}".format(
                    Traintracker.valid_accs_robust[-1]
                    if Traintracker.valid_accs_robust
                    else 0
                )
            )
            acc_c = compute_c_corruptions(
                args.dataset,
                testsets_c,
                net,
                batchsize=100,
...
if __name__ == "__main__":
    ...
    testsets_c = (
        Dataloader.load_data_c(subset=args.validonc, subsetsize=100)
        if args.validonc
        else None
    )
    ...
    if args.warmupepochs > 0:
        warmupscheduler = optim.lr_scheduler.LinearLR(
            optimizer, start_factor=0.1, end_factor=1.0, total_iters=args.warmupepochs
        )
        scheduler = optim.lr_scheduler.SequentialLR(
            optimizer,
            schedulers=[warmupscheduler, scheduler],
    ...
    checkpoint_save_path = os.path.join(
        checkpoint_dir, f"checkpoint_{args.experiment}.pt"
    )

    Checkpointer = utils.Checkpoint(
        args.combine_train_corruptions,
        args.dataset,
        args.modeltype,
        args.experiment,
        train_corruptions,
    ...
    Traintracker = utils.TrainTracking(
        args.dataset,
        args.modeltype,
        args.lrschedule,
        args.experiment,
    ...
    if args.resume == True:
        start_epoch, model, swa_model, optimizer, scheduler, swa_scheduler = (
            Checkpointer.load_model(
                model, swa_model, optimizer, scheduler, swa_scheduler, "standard"
            )
        )
        Traintracker.load_learning_curves()
        print("\nResuming from checkpoint after epoch", start_epoch)
    # load augmented trainset and Dataloader
    Dataloader.load_augmented_traindata(
        target_size=len(Dataloader.base_trainset),
        epoch=start_epoch,
    ...
    if (
        ...
        style_dataloader = Dataloader.load_style_dataloader(style_dir=args.style_dir)
    ...
    with tqdm(total=total_steps, initial=start_steps) as pbar:
        with torch.autograd.set_detect_anomaly(
            ...
            for epoch in range(start_epoch, end_epoch):
                ...
                valid_acc, valid_loss, valid_acc_robust, valid_acc_adv = valid_epoch(
                    pbar, model
                )
                ...
                if args.swa["apply"] == True and (epoch + 1) > swa_start:
                    ...
                    (
                        ...
                        valid_acc_adv_swa,
                    ) = valid_epoch(pbar, swa_model)
                else:
                    if args.lrschedule == "ReduceLROnPlateau":
                        scheduler.step(valid_loss)
                    else:
                        scheduler.step()
                    valid_acc_swa, valid_acc_robust_swa, valid_acc_adv_swa = (
                        valid_acc,
                        valid_acc_robust,
                        valid_acc_adv,
                    )
                ...
                Checkpointer.save_checkpoint(
                    model, swa_model, optimizer, scheduler, swa_scheduler, epoch
                )
                Traintracker.save_metrics(
                    train_acc,
                    valid_acc,
                    valid_acc_robust,
                    valid_acc_adv,
    ...
    if args.swa["apply"] == True:
        ...
        if criterion.robust_samples >= 1:
            SWA_Loader = data.SwaLoader(
                trainloader, args.batchsize, criterion.robust_samples
            )
