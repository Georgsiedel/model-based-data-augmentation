...
class SwinTransformer(ct_model.CtModel):
    """
    ...
    def __init__(
        ...
        super(SwinTransformer, self).__init__(dataset=dataset, normalized=normalized, num_classes=num_classes)      
    ...
    def forward(self, x, targets=None, robust_samples=0, corruptions=None, mixup_alpha=0.0, mixup_p=0.0, manifold=False, 
        ...
        x, mixed_targets = super(SwinTransformer, self).noise_mixup(x, targets, robust_samples, corruptions, mixup_alpha, 
                                    ...
                                    noise_sparsity, noise_patch_lower_scale, noise_patch_upper_scale, generated_ratio)
        x = self.features(x)
        x = self.norm(x)
        x = self.permute(x)
        x = self.avgpool(x)
...
def swin_v2_t(*, weights: Optional[Swin_V2_T_Weights] = None, dataset: str, normalized: bool, num_classes: int, progress: bool = True, **kwargs: Any) -> SwinTransformer:
    """
