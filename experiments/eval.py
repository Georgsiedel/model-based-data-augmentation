...
parser.add_argument(
    ...
    default=0,
    type=int,
    help="How many workers are launched to parallelize data loading.",
)
parser.add_argument(
    "--normalize",
    type=utils.str2bool,
...
parser.add_argument(
    "--autoattack_params",
    default={"setsize": 500, "epsilon": 8 / 255, "norm": "Linf"},
    type=str,
    action=utils.str2dictAction,
    metavar="KEY=VALUE",
    help="parameters for the trades loss function",
...
def compute_clean(testloader, model, num_classes):
    with torch.no_grad():
        ...
        calibration_metric = MulticlassCalibrationError(
            num_classes=num_classes, n_bins=15, norm="l2"
        )
        all_targets = torch.empty(0)
        all_targets_pred = torch.empty((0, num_classes))
        all_targets, all_targets_pred = (
            all_targets.to(device),
            all_targets_pred.to(device),
        )
...
if __name__ == "__main__":
    Testtracker = utils.TestTracking(
        args.dataset,
        args.modeltype,
        args.experiment,
        args.runs,
        args.combine_test_corruptions,
        args.test_on_c,
        args.calculate_adv_distance,
        args.calculate_autoattack_robustness,
        test_corruptions,
        args.adv_distance_params,
    )
    ...
    for run in range(args.runs):
        ...
        state_dict = torch.load(Testtracker.filename, weights_only=True)[
            "model_state_dict"
        ]
        ...
        if args.test_on_c == True:  # C-dataset robust accuracy
            ...
            subsetsize = (
                None if args.validontest else 1000
            )  # subset for quick validation runs

            testsets_c = Dataloader.load_data_c(
                subset=subset, subsetsize=subsetsize, valid_run=False
            )
            accs_c = eval_corruptions.compute_c_corruptions(
                args.dataset,
                testsets_c,
                model,
                args.batchsize,
        ...
        if args.calculate_adv_distance == True:  # adversarial distance calculation
            adv_acc, dist_sorted, mean_dist = eval_adversarial.compute_adv_distance(
                Dataloader.testset, 0, model, args.adv_distance_params
            )
        ...
        if (
            args.calculate_autoattack_robustness == True
        ):  # adversarial accuracy calculation
            adv_acc_aa, mean_dist_aa = eval_adversarial.compute_adv_acc(
                args.autoattack_params, Dataloader.testset, model, 0, args.batchsize
            )
            Testtracker.track_results([adv_acc_aa, mean_dist_aa])

        # Robust Accuracy on p-norm noise - either combined or separate noise types
        accs = eval_corruptions.select_p_corruptions(
            testloader,
            model,
            test_corruptions,
            args.dataset,
