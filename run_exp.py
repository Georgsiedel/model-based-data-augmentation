...
if __name__ == "__main__":
    ...
    os.environ["CUDA_LAUNCH_BLOCKING"] = (
        "1"  # prevents "CUDA error: unspecified launch failure" and is recommended for some illegal memory access errors #increases train time by ~5-15%
    )

    for experiment in [0]:  # range(95,101):
        ...
        for run in range(runs):
            ...
            if config.combine_train_corruptions:
                ...
                for id, (train_corruption) in enumerate(config.train_corruptions):
                    ...
                    cmd0 = (
                        f'python experiments/train.py --resume={resume} --train_corruptions="{train_corruption}"'
        ...
        cmdeval = (
            ...
            '--calculate_autoattack_robustness={} --autoattack_params="{}" --combine_train_corruptions={} --kaggle={} '.format(
                ...
                config.modelparams,
                config.resize,
                config.combine_test_corruptions,
                0,
                config.normalize,
                config.pixel_factor,
                config.test_on_c,
                config.calculate_adv_distance,
                config.adv_distance_params,
